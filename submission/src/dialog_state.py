...
class DialogState:
    ...
    def __init__(self, llm=None, freeform: bool = False) -> None:
        ...
        # unless the operator configured a key and a mode -- see src/llm.py.
        self.llm = llm
        ...
        # of `facet_values()` skips its facet block entirely. See src/facets.py.
    ...
    def observe(self, message: str, turn: int) -> None:
        ...
        if disclosure:
            if override:
                ...
                # on routes 1 and 2 (`src/retrieval.py:384,391`), so a stale ceiling
    ...
    def next_attribute(self) -> str | None:
        ...
        for attribute in ASK_ORDER:
            if attribute not in self.exhausted:
                ...
                # Called exactly once per turn (src/agent.py:150).
