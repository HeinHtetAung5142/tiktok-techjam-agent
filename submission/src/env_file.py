...
Why this exists
---------------
`src/llm.py` is configured entirely from environment variables (a hard rule: no key
...
1. **Nothing on the scored path imports this module.** `src/agent.py`,
...
# never truncates. Kept in sync with `src/llm.py`; imported rather than re-typed so the
# two cannot drift.
MANAGED_KEYS = (
    llm.MODE_ENV,
    llm.API_KEY_ENV,
