# Shopping Copilot — TikTok TechJam 2026 submission
...
`numpy`, `scipy` and `scikit-learn`, used by the dense-retrieval route. They are the
**only** third-party dependencies; everything else is standard library.

**This step is required, and the failure mode is quiet.** The dense index imports lazily
inside a broad `try/except`, so a missing stack degrades to sparse-only retrieval rather
than crashing. The only symptom is one line on stderr:
...
### Step 2 — Provide the catalog

`data/catalog.jsonl` (50,000 rows, the frozen official catalog) must be present. It is not
included in this bundle: `docs/submission_rules.md` bars shipping evaluation data, and the
organizer already holds the frozen artifact.
...
> fail. Use the real launcher — `py` — which is what the commands below assume. On
...
### One command, in the official harness

```bash
py -m evaluator.local_evaluator
```

The harness imports the agent as `from starter.agent import Agent`. This bundle satisfies
that path *and* the layout `docs/submission_rules.md` recommends — `starter/agent.py` here
is a four-line shim re-exporting the same class as the canonical `agent.py`. There is one
implementation, in `src/`, not two copies.

```python
from agent import Agent          # canonical entry point
from starter.agent import Agent  # same class, harness-compatible path
```
...
**Runs are deterministic.** The evaluator regenerates intent cards with a seeded RNG, so
identical code always scores identically, bit for bit. A changed score means a changed
agent — never run-to-run noise.
...
`budget`, `feature`, `use_case`, `other`, or `null`. Recommendations are ordered
best-first; only the first 10 valid, unique, in-catalog ids are scored.

---

## 4. Feasibility disclosure
